...
@section('page-title')
    {{ __('Import Expenses - Column Mapping') }}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('expense.index') }}">{{ __('Expense') }}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('expense.import.index') }}">{{ __('Import') }}</a></li>
                ...
                <form id="mapping-form">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            @foreach(['property_name', 'unit_name', 'expense_type', 'amount', 'date', 'title'] as $field)
                        ...
                        <div class="col-md-6">
                            ...
                            @foreach(['expense_id', 'notes'] as $field)
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="{{ route('expense.import.cancel') }}" class="btn btn-secondary">
...
$(document).ready(function() {
    $('#mapping-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("expense.import.validate") }}',
            type: 'POST',
            data: formData,
    ...
    function displayValidationResults(validation) {
        ...
        html += '{{ __("Expenses to be created:") }} <strong>' + (validation.tenants_to_create ? validation.tenants_to_create.length : 0) + '</strong><br>';
    ...
    function loadUnitsForProperty(rowKey, propertyId) {
        ...
        $.ajax({
            url: '{{ route("expense.import.units") }}',
            type: 'GET',
    ...
    function executeImport() {
        ...
        $.ajax({
            url: '{{ route("expense.import.execute") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = '{{ route("expense.import.result") }}';
                } else {
