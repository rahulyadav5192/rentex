    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                @php
                    $importResult = $result ?? [];
                    $invoicesCreated = $importResult['invoices_created'] ?? 0;
                    ...
                    <div class="col-md-6">
                        <div class="card border-0 shadow-sm">
                            <div class="card-body text-center">
                                <h2 class="text-success mb-2">{{ $invoicesCreated }}</h2>
                                <p class="mb-0 text-muted">{{ __('Invoices Created') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="alert alert-warning">
                        ...
                        <ul class="mb-0 mt-2">
                            @foreach($skipReasons as $reason)
                                <li>{{ __('Row') }} {{ $reason['row'] }}: {{ $reason['reason'] }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif
                ...
                <div class="d-flex gap-2 mt-4">
                    <a href="{{ route('invoice.index') }}" class="btn btn-primary">
                        <i class="ti ti-list me-2"></i>{{ __('View Invoices') }}
                    </a>
                    <a href="{{ route('invoice.import.index') }}" class="btn btn-secondary">
