                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="alert alert-info">
                            ...
                            <br>
                            <strong>{{ __('File structure:') }}</strong> Each row represents one invoice. Property and unit names will be matched automatically (case-insensitive). Invoice types will be created automatically if they don't exist.
                        </div>
                        ...
                        <div class="alert alert-warning">
                            ...
                            <ul class="mb-0">
                                ...
                                <li>{{ __('Invoice types will be created automatically if they don\'t exist') }}</li>
                                <li>{{ __('Date fields support multiple formats: YYYY-MM-DD, YYYY-MM, DD/MM/YYYY, MM/DD/YYYY, Excel date serial numbers, and many other common date formats') }}</li>
                                <li>{{ __('Invoice ID field: If you provide values like "INV-001", the system will extract the numeric part (001) automatically') }}</li>
                            </ul>
                        </div>
...
$(document).ready(function() {
    $('#import-upload-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("invoice.import.upload") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function(response) {
                if (response.success) {
                    window.location.href = '{{ route("invoice.import.mapping") }}';
                } else {
