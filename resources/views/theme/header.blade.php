    ...
    <div class="max-w-7xl mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-white/40 dark:border-slate-700/50 rounded-full shadow-lg dark:shadow-slate-950/50 flex justify-between items-center px-6 py-3 transition-all duration-300">
        <div class="flex-shrink-0 flex items-center gap-2 cursor-pointer" onclick="window.location.href='{{ route('web.page', $user->code) }}'">
            @if (!empty($logoUrl1) && $logoUrl1 != asset('logo.png'))
                <img src="{{ $logoUrl1 }}" alt="Logo" class="h-8 w-auto max-w-[180px] object-contain" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
            @endif
            <div class="flex items-center gap-2" style="{{ !empty($logoUrl1) && $logoUrl1 != asset('logo.png') ? 'display: none;' : '' }}">
        ...
        <div class="hidden md:flex items-center space-x-1">
            <a class="px-5 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all duration-200 {{ in_array($routeName, ['web.page']) ? 'text-primary dark:text-primary bg-slate-100 dark:bg-slate-800/50' : '' }}" 
               href="{{ route('web.page', $user->code) }}">
                {{ __('Home') }}
            </a>
            <a class="px-5 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all duration-200 {{ in_array($routeName, ['property.home','property.detail']) ? 'text-primary dark:text-primary bg-slate-100 dark:bg-slate-800/50' : '' }}" 
               href="{{ route('property.home', ['code' => $user->code]) }}">
                {{ __('Properties') }}
            </a>
            <a class="px-5 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all duration-200 {{ in_array($routeName, ['blog.home','blog.detail']) ? 'text-primary dark:text-primary bg-slate-100 dark:bg-slate-800/50' : '' }}" 
               href="{{ route('blog.home', ['code' => $user->code]) }}">
                {{ __('Blog') }}
            </a>
            <a class="px-5 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all duration-200 {{ in_array($routeName, ['contact.home']) ? 'text-primary dark:text-primary bg-slate-100 dark:bg-slate-800/50' : '' }}" 
        ...
        <div class="hidden md:flex items-center gap-3">
            ...
            @if (!empty($Section_0_content_value['btn_name'] ?? ''))
                <a class="bg-slate-900 dark:bg-slate-800 text-white dark:text-white px-6 py-2.5 rounded-full text-sm font-bold hover:bg-slate-800 dark:hover:bg-slate-700 transition-all shadow-lg hover:shadow-lg transform hover:-translate-y-0.5 border border-slate-800 dark:border-slate-700" 
                   href="{{ $Section_0_content_value['btn_link'] ?? '#' }}">
                    {{ $Section_0_content_value['btn_name'] ?? __('Explore') }}
                </a>
            @else
                <a class="bg-slate-900 dark:bg-slate-800 text-white dark:text-white px-6 py-2.5 rounded-full text-sm font-bold hover:bg-slate-800 dark:hover:bg-slate-700 transition-all shadow-lg hover:shadow-lg transform hover:-translate-y-0.5 border border-slate-800 dark:border-slate-700" 
                   href="{{ route('property.home', ['code' => $user->code]) }}">
