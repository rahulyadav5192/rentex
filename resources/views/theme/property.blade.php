    ...
    <div class="bg-background-light dark:bg-background-dark pt-32 md:pt-36 pb-6" style="padding-top: 110px !important;">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 class="text-3xl md:text-4xl font-semibold text-text-main-light dark:text-text-main-dark mb-3 tracking-tight">
                        {{ !empty($Section_3_content_value['sec3_title']) ? $Section_3_content_value['sec3_title'] : __('Explore Available Properties Instantly') }}
                    </h1>
                    <p class="text-lg text-text-sub-light dark:text-text-sub-dark max-w-2xl font-light">
                        {{ !empty($Section_3_content_value['sec3_sub_title']) ? $Section_3_content_value['sec3_sub_title'] : __('Your complete property overview with photos, amenities, and key details.') }}
                    </p>
                </div>
                @if (!empty($listingTypes) && count($listingTypes) > 0)
                    <div class="flex bg-gray-200 dark:bg-gray-800 rounded-full p-1 shadow-inner">
                        <button class="px-6 md:px-8 py-2.5 rounded-full font-medium shadow-sm transition-all text-sm {{ $currentListingType == 'all' ? 'bg-primary text-white' : 'text-text-sub-light dark:text-text-sub-dark hover:text-primary dark:hover:text-white' }}"
                                onclick="switchListingType('all', this)"
                                data-listing-type="all">
                            {{ __('All') }}
                        </button>
                        @foreach ($listingTypes as $type)
                            <button class="px-6 md:px-8 py-2.5 rounded-full font-medium shadow-sm transition-all text-sm {{ $currentListingType == $type ? 'bg-primary text-white' : 'text-text-sub-light dark:text-text-sub-dark hover:text-primary dark:hover:text-white' }}"
                                    onclick="switchListingType('{{ $type }}', this)"
                                    data-listing-type="{{ $type }}">
                                {{ $type == 'rent' ? __('Rent') : __('Buy') }}
                            </button>
                        @endforeach
                    </div>
                @endif
        ...
        <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-card-light dark:bg-card-dark rounded-xl shadow-soft dark:shadow-none dark:border dark:border-border-dark p-6 md:p-8 transition-all duration-300">
                {{ Form::open(['route' => ['search.filter', 'code' => $user->code], 'method' => 'GET', 'id' => 'package_filter', 'class' => 'flex flex-col xl:flex-row gap-6 items-end']) }}
                
                <!-- Hidden field for listing_type -->
                <input type="hidden" name="listing_type" id="listing_type_filter" value="{{ $currentListingType }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#country').on('change', function () {
                ...
                $.ajax({
                    url: "{{ route('get-states', ['code' => $user->code]) }}",
                    type: 'GET',
            ...
            $('#state').on('change', function () {
                ...
                $.ajax({
                     url: "{{ route('get-cities', ['code' => $user->code]) }}",
                    type: 'GET',
                    data: { state: state },
                    success: function (res) {
