    ...
    <div class="bg-background-light dark:bg-background-dark pt-32 md:pt-36 pb-6" style="padding-top: 110px !important;">
                ...
                <div class="flex items-center gap-3 flex-shrink-0">
                    @if (!empty($listingTypes) && count($listingTypes) > 0)
                        <div class="flex bg-gray-200 dark:bg-gray-800 rounded-full p-1 shadow-inner">
                            <button class="px-4 sm:px-6 md:px-8 py-2 sm:py-2.5 rounded-full font-medium shadow-sm transition-all text-xs sm:text-sm {{ $currentListingType == 'all' ? 'bg-primary text-white' : 'text-text-sub-light dark:text-text-sub-dark hover:text-primary dark:hover:text-white' }}"
                                    onclick="switchListingType('all', this)"
                                    data-listing-type="all">
                                {{ __('All') }}
                            </button>
                            @foreach ($listingTypes as $type)
                                <button class="px-4 sm:px-6 md:px-8 py-2 sm:py-2.5 rounded-full font-medium shadow-sm transition-all text-xs sm:text-sm {{ $currentListingType == $type ? 'bg-primary text-white' : 'text-text-sub-light dark:text-text-sub-dark hover:text-primary dark:hover:text-white' }}"
        ...
        <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div id="filterSection" class="bg-card-light dark:bg-card-dark rounded-xl shadow-soft dark:shadow-none dark:border dark:border-border-dark p-4 sm:p-6 md:p-8 transition-all duration-300 overflow-hidden hidden">
                {{ Form::open(['route' => ['search.filter', 'code' => $user->code], 'method' => 'GET', 'id' => 'package_filter', 'class' => 'flex flex-col lg:flex-row gap-4 sm:gap-6 items-end']) }}
