    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        @foreach ($blogs as $blog)
            ...
            <article class="group blog-card rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2" style="background-color: white; border: 1px solid #e2e8f0;">
                <a href="{{ route('blog.detail', ['code' => $user->code, 'slug' => $blog->slug]) }}" class="block">
        ...
        <div class="mt-12 md:mt-16">
            <nav class="flex flex-col items-center gap-4">
                <ul class="flex flex-wrap items-center justify-center gap-2">
                    ...
                    @foreach ($blogs->links()->elements[0] as $page => $url)
                        ...
                        @else
                            <li>
                                <a href="{{ $url }}" 
                                   class="px-4 py-2 rounded-lg {{ $page == $blogs->currentPage() ? 'bg-primary text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-primary hover:text-white dark:hover:bg-primary' }} transition-all shadow-md hover:shadow-lg">
                                    {{ $page }}
                                </a>
                            </li>
                        @endif
                ...
                <p class="text-sm text-slate-600 dark:text-slate-400">
                    {{ __('Showing') }} {{ ($blogs->currentPage() - 1) * $blogs->perPage() + 1 }} – 
                    {{ min($blogs->currentPage() * $blogs->perPage(), $blogs->total()) }} 
                    {{ __('of') }} {{ $blogs->total() }} {{ __('posts') }}
                </p>
            </nav>
        </div>
    @endif
@else
    <div class="text-center py-16 md:py-24">
        <div class="max-w-md mx-auto">
            ...
            <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">{{ __('No Blog Posts Yet') }}</h3>
            <p class="text-slate-600 dark:text-slate-400">{{ __('Check back soon for new content!') }}</p>
        </div>
    </div>
@endif
